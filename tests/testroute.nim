...
suite "Generic REST API router & macro tests, body type Option[ContentBody]":
  test "No parameters test":
    var router = RestRouterGen[Option[ContentBody]].init(testValidate)
    router.api(MethodGet, "/test/empty/1") do (
      ) -> RestApiResponse:
      return RestApiResponse.response("ok-1", contentType = "test/test")
    router.api(MethodGet, "/test/empty/p1/2") do (
  ...
  test "Routes installation from generic proc":
    proc addGenericRoute(router: var RestRouterGen[Option[ContentBody]], T: type) =
      const typeName = typetraits.name(T)
      router.api(MethodGet, "/test/" & typeName) do () -> RestApiResponse:
        return RestApiResponse.response(typeName, contentType = "text/plain")
...
suite "Generic REST API router & macro tests, body type Opt[ContentBody]":
  test "No parameters test":
    var router = RestRouterGen[Opt[ContentBody]].init(testValidate)
    router.api(MethodGet, "/test/empty/1") do (
      ) -> RestApiResponse:
      return RestApiResponse.response("ok-1", contentType = "test/test")
    router.api(MethodGet, "/test/empty/p1/2") do (
  ...
  test "Routes installation from generic proc":
    proc addGenericRoute(router: var RestRouterGen[Opt[ContentBody]], T: type) =
      const typeName = typetraits.name(T)
      router.api(MethodGet, "/test/" & typeName) do () -> RestApiResponse:
        return RestApiResponse.response(typeName, contentType = "text/plain")
  ...
  test "Custom types as parameters test":
    ...
    router.api(MethodPost,
               "/test/custom_args/1/{pat1}/{pat2}/{pat3}") do (
      pat1: CustomType1, pat2: CustomType1,
      pat3: CustomType1, body: Opt[ContentBody]) -> RestApiResponse:
        let p1 = pat1.get()
        let p2 = pat2.get()
        let p3 = pat3.get()
        let cbody = body.get()
  ...
  test "seq[basic] types parameters test":
    ...
    router.api(MethodPost, "/test/basic_seq/1/{smp3}") do (
      opt1: seq[int], opt2: seq[string], opt3: seq[seq[byte]], opt4: seq[int],
      smp3: seq[byte], body: Opt[ContentBody]) -> RestApiResponse:
        let p1 = smp3.get()
        let o1 = opt1.get()
        let o2 = opt2.get()
        let o3 = opt3.get()
  ...
  test "Unique routes test":
    ...
    proc apiCallback(
      request: HttpRequestRef, pathParams: HttpTable,
      queryParams: HttpTable,
      body: Opt[ContentBody]): Future[RestApiResponse] {.
        async: (raises: [CancelledError]).} =
        discard
